...
echo Benchmark Owner User Created. ID: $BENCHMARK_OWNER
...
echo Model Owner User Created. Id: $MODEL_OWNER
...
echo Data Owner User Created. ID: $DATA_OWNER
...
# Creata a new benchmark by Benchmark owner
...
RESULT1=$(curl -s -X POST "http://127.0.0.1:8000/results/" -H  "accept: application/json" -H  "Authorization: Token $DATASET_OWNER_TOKEN" -H  "Content-Type: application/json" -d "{  \"name\": \"result1\",  \"results\": {\"key1\":\"value1\", \"key2\":\"value2\"},  \"metadata\": {},  \"approval_status\": \"APPROVED\",  \"benchmark\": $BENCHMARK,  \"model\": $MODEL_EXECUTOR1_MLCUBE,  \"dataset\": $DATASET}")

echo Result created for Benchmark ID: $BENCHMARK  Model ID: $MODEL_EXECUTOR1_MLCUBE Dataset ID: $DATASET

# Create new result for the dataset with model_executor2 mlcube and benchmark 
RESULT2=$(curl -s -X POST "http://127.0.0.1:8000/results/" -H  "accept: application/json" -H  "Authorization: Token $DATASET_OWNER_TOKEN" -H  "Content-Type: application/json" -d "{  \"name\": \"result2\",  \"results\": {\"key1\":\"value1\", \"key2\":\"value2\"},  \"metadata\": {},  \"approval_status\": \"APPROVED\",  \"benchmark\": $BENCHMARK,  \"model\": $MODEL_EXECUTOR2_MLCUBE,  \"dataset\": $DATASET}")

echo Result created for Benchmark ID: $BENCHMARK  Model ID: $MODEL_EXECUTOR2_MLCUBE Dataset ID: $DATASET
